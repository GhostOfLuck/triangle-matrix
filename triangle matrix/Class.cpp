...
matrix::matrix(int row1, int colume1)
{
	...
	this->row =row1;
	this->colume = colume1;
	matr = new double *[row1];
	for (int i = 0; i < row1; i++)
		for (int j = 0; j < row1 - i; j++)
		//for (int j = row1 - i - 1; j < row1; j++)
		{
			matr[j] = new double[row1];
		}
	for (int i = 0; i < row1; i++)
...
{
	double c = 0;
	for (int r = 0; r < colume - i && r <colume - j; r++)
	{
		//for (int k = 0; k < colume - i; k++)
		//if (matr[i][r] * x.matr[r][j]>0)
			//if ((int)(matr[i][r] * x.matr[r][j])== matr[i][r] * x.matr[r][j])
...
istream& operator >> (istream &in, matrix &x)
{
	...
	{
		for (int j = 0; j < x.getRow() - i ; j++)
		//for (int j = x.getRow() - i - 1; j < x.getColume(); j++)
		{
			in >> element;
...
ostream& operator << (ostream &out, matrix x)
{
	for (int i = 0; i < x.getRow(); i++)
	{
		//for (int k = 0; k < x.getRow() - i; k++)
			//cout << setw(x.getColume()-i);
		for (int j = 0; j < x.getRow() - i; j++)
		//for (int j = x.getRow() - i - 1; j < x.getColume(); j++)
		{
			out << setw(10)<< x.getElement(i,j);
		}
		out << endl;
	}
